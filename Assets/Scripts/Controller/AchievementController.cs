...
public class AchievementController : MonoBehaviour
{
    ...
    // Untuk memudahkan panggilan script yang bersifat manager dari script lain
    ...
    private void Update()
    {
        if (_popUpShowDurationCounter > 0)
        {
            ...
            _popUpTransform.localScale = Vector2.LerpUnclamped(
                _popUpTransform.localScale, Vector2.one, 0.5f);
        }
        else
        {
            _popUpTransform.localScale = Vector2.LerpUnclamped(
                _popUpTransform.localScale, Vector2.right, 0.5f);
        }
    }
...
// dan bisa di-inputkan dari inspector, jika tidak terdapat ini, maka variable
